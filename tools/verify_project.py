...
def main() -> int:
    ...
    require('server/base_url="https://shadowrift.oakgatekeeper.uk"' in project_settings, "custom authority domain missing")
    require('server/fallback_url="https://shadowrift-authority.kim-phong619.workers.dev"' in project_settings, "authority fallback endpoint missing")
    ...
    require("FALLBACK_SERVER_URL_SETTING" in client and "_switch_to_fallback_endpoint" in client and "dns_failed" in client, "authority DNS fallback missing")
    ...
    require("PASS: 16 behavior tests" in tests and "_test_full_scene_boot_and_pause" in tests, "final Godot runtime coverage missing")
    ...
    require("_test_authority_endpoint_fallback_configuration" in tests and "dns_failed" in tests, "authority endpoint fallback coverage missing")
    ...
    wrangler = (ROOT / "server/wrangler.jsonc").read_text(encoding="utf-8")
    require('"workers_dev": true' in wrangler, "workers.dev fallback must stay enabled")
    require('"pattern": "shadowrift.oakgatekeeper.uk"' in wrangler and '"custom_domain": true' in wrangler, "authority custom domain route missing")
