...
- Hero death now completes its authored animation, locks gameplay input, presents a `DEFEATED` overlay, and resumes only through `RETRY` at the last checkpoint with restored HP/MP and short invulnerability. Lethal damage explicitly forces both Hero and articulated rig visible/white before entering Death, preventing the invulnerability-blink state from freezing the character invisible; Retry resets the same visual state. Dead saves are normalized to a playable recovery state; enemy/boss cleanup still waits for complete death animations.
...
- Articulated cutout generation now partitions source alpha exclusively between body parts. Hero lower-leg masks start below the hip/cape region and locomotion uses only ±3° leg rotation plus opposite-phase translation, removing the remaining painted-cutout smear/double-image effect during running.
...
- Wraith is now a ranged caster: it retreats when crowded and maintains a mid-range casting band. Its cast telegraph is now 0.72s with a 1.55s cooldown; the cyan/violet bolt travels at 285 px/s, homes for only 0.38s with a 105°/s turn cap, then commits ballistically so touch players can dodge rather than being chased indefinitely. Combat faction checks reject enemy friendly fire.
- Save/checkpoint persistence is now JSON-safe (`{x,y}`) with backward parsing for legacy schema-2 `"(x, y)"` checkpoint strings while preserving checksum compatibility. Death-plane Killzones are terminal, stage advancement is token-guarded/idempotent against timer+exit races, gameplay transition/death timers pause with the SceneTree, LevelExit re-arms after early entry, desktop/editor mouse touch emulation is enabled, and projectile pool release is deferred out of physics query callbacks.
