...
Decision: combat Hurtbox/Hitbox areas remain separate from physical world bodies, but Hero↔enemy/boss bodies receive mutual collision exceptions to avoid overlap traps. Hero receives 0.78 s post-hit invulnerability with a shorter 0.18 s Hurt lock and stronger separation knockback. Wraith is a mid-range caster using the existing projectile pool. Device QA showed unlimited damped homing was too punishing, so the donor steering pattern is intentionally constrained: 0.72 s cast, 1.55 s cooldown, 285 px/s bolt, 0.38 s lock window and 105°/s maximum turn rate; after lock expiry the bolt is ballistic and cannot U-turn.
...
Device follow-up (2026-09-03): Hero death now forces the parent and rig visible before Death and again at Retry, preventing invulnerability blink from freezing an invisible lethal frame. For painted cutout locomotion, lower-leg masks were narrowed below the cape/hip and run animation changed from ~30° leg swings to ±3° plus opposite-phase translations; this keeps articulation while avoiding alpha-resampling smear.
